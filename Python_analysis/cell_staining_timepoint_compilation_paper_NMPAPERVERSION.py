...
expt_ids = [['/210416_FB2_HADA_Staining', '/220121_bFB66_Tun_gr_HADA', '/220104_bFB66_Tun_HADA', '/240126_bFB66_Tun_HADA'], ['/240202_bFB69_Tun_HADA','/241113_bFB69_Tun_HADA', '/220204_bFB69_Tun_gr_HADA'], ['/220830_bFB145_Tun_gr_HADA', '/220909_bFB145_Tun_gr_HADA']]
data_loc=[['data_ssd1', 'data_ssd1', 'data_ssd1', 'data_ssd1'], ['data_ssd1', 'data_ssd1', 'data_ssd1'], ['data_ssd2', 'data_ssd2']]
group_id = 'WT_ponA_comp_spots'
pert = 'Tunicamycin added'
plotting_timepoints=[[0,10, 20, 30, 50],[0,10, 20, 30, 50], [50]]
spots=True
...
if not os.path.exists(out_name):
    ...
    for ind in range(len(expt_ids)):
        ...
        timepoint_search_terms = ['LB', '0 min', '5 min', '10 min', '15 min', '20 min', '25 min', '30 min', '35 min',
                                  '40 min', '45 min', '50 min', '55 min', '60 min', 'Xylose induced', '120 min']
...
def iter_plotting_v1(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel, temp_hue=None, plot_points=True):
    ...
    if plot_points:
        plot = sns.stripplot(x=temp_iter, y=temp_var, dodge=True, data=df, edgecolor='black', linewidth=1, alpha=0.2,
                             hue=temp_hue)
...
def iter_plotting_v2(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel, temp_hue=None, plot_points=True):
    ...
    plot = sns.lineplot(x=temp_iter, y=temp_var, data=df, hue='Celltype', estimator=np.mean, err_style='bars',
                        markers=True, err_kws={'capsize': 5.0})
...
# These are iterable
temp_vars = ['Average outline ' + label, 'Average outline ' + label, 'Average ' + label, 'CV outline ' + label,
             'SD outline ' + label, 'Skew outline ' + label]
# temp_vars=['Average outline '+label, 'Average outline '+label, 'Average '+label, 'Normalized Average outline '+label]
temp_out_names = ['av_outline_fl', 'av_outline_fl_labs', 'av_area_fl', 'CV_outline_fl', 'SD_outline_fl',
                  'Skew_outline_fl']
temp_ylabs = ['Fluorescence', 'FDAA fluorescence', 'Area fluorescence', 'CV Fluorescence', 'SD fluorescence',
              'Skew fluroescence']
if norm_cond != 0:
...
sns.set(font_scale=1.5)
sns.set_style("whitegrid")
fig, ax = iter_plotting_v1(df[df.Condition != excl], temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel,
                           plot_points=False, temp_hue=hue)
...
def iter_plotting_v3(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel,err, temp_hue=None, plot_points=False):
    fig = plt.figure(figsize=[2.5, 2])
    sns.set(font_scale=0.9)
    sns.set_style("ticks")
    ax = plt.subplot(1, 1, 1)
    hue_order = list(df[temp_hue].unique())
    plot = sns.lineplot(x=temp_iter, y=temp_var, data=df, hue=temp_hue, estimator=np.mean, err_style='bars',
                        errorbar=err,
...
for i0 in range(len(temp_vars)):
    temp_var, temp_out_name, temp_ylab = temp_vars[i0], temp_out_names[i0], temp_ylabs[i0]
    fig, ax = iter_plotting_v3(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel, temp_hue=hue,err=errorbars)
    ...
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_legend.png', dpi=300,
                bbox_inches='tight')
...
for i0 in range(len(temp_vars)):
    ...
    fig, ax = iter_plotting_v3(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel, temp_hue=hue,
                               plot_points=True,err=errorbars)
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_plotpoints.eps', dpi=300,
                bbox_inches='tight')
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_plotpoints.png', dpi=300,
                bbox_inches='tight')
    # plt.legend()
    # plt.legend(loc=[0.6, 0.6])
    # plt.legend(loc=[0.4, 0.55])
    # plt.legend(loc=[0.5, 0.4])
    plt.legend(loc=[0.01, 0.93])
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_legend_plotpoints.png', dpi=300,
                bbox_inches='tight')
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_legend_plotpoints.eps', dpi=300,
                bbox_inches='tight')
    plt.clf()
    print('Finished plot {0}'.format(i0))
...
for i0 in range(len(temp_vars)):
    temp_var, temp_out_name, temp_ylab = temp_vars[i0], temp_out_names[i0], temp_ylabs[i0]
    fig, ax = iter_plotting_v3(df, temp_var, temp_out_name, temp_ylab, temp_iter, temp_xlabel, temp_hue='Date',err=errorbars)
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact.eps', dpi=300, bbox_inches='tight')
    # plt.legend()
    plt.legend(loc=[0.6, 0.6])
    fig.savefig(output_dir + group_id + '_' + temp_out_name + '_timeplot_compact_legend_expt.png', dpi=300,
                bbox_inches='tight')
    plt.clf()
    print('Finished plot {0}'.format(i0))
